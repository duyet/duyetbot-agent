      ...
      return {
          ...
          // UPDATE
            ...
            if (sessionIndex >= 0) {
              ...
              if (sql.includes("state =")) {
                data.sessions[sessionIndex]!.state = boundValues[
                  valueIndex++
                ] as string;
              }
              if (sql.includes("title =")) {
                data.sessions[sessionIndex]!.title = boundValues[
                  valueIndex++
                ] as string | null;
              }
              if (sql.includes("metadata =")) {
                data.sessions[sessionIndex]!.metadata = boundValues[
                  valueIndex++
                ] as string | null;
              }
        ...
        async first() {
          ...
          if (sql.includes("COUNT(*)")) {
            ...
            let count = data.sessions.filter(
              (s) => s.user_id === userId,
            ).length;
            ...
            if (sql.includes("AND state = ?") && boundValues.length > 1) {
              ...
              count = data.sessions.filter(
                (s) => s.user_id === userId && s.state === state,
              ).length;
            }

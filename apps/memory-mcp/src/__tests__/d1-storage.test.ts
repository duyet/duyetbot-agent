      ...
      bind: vi.fn((...args: unknown[]) => ({
        ...
        all: vi.fn(async <T>() => {
          if (sql.includes("FROM memory_sessions WHERE user_id")) {
            const userId = args[0];
            const results = Array.from(data.sessions.values()).filter(
              (s) => s.user_id === userId,
            );
          ...
          if (sql.includes("FROM memory_messages m")) {
            ...
            for (const [sessionId, msgs] of data.messages) {
              ...
              for (let i = 0; i < msgs.length; i++) {
                if (
                  msgs[i].content.toLowerCase().includes(query.toLowerCase())
                ) {
        ...
        run: vi.fn(async () => {
          ...
          if (
            sql.includes("DELETE FROM memory_session_tokens WHERE expires_at")
          ) {

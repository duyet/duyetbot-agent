...
export function useSessionPersistence(chatId: string) {
  ...
  const loadSession = useCallback((): SessionState | null => {
    try {
      const saved = localStorage.getItem(`${STORAGE_KEYS.ACTIVE_SESSION}:${chatId}`);
      if (saved) {
  ...
  const loadDraft = useCallback((): string => {
    try {
      const saved = localStorage.getItem(`${STORAGE_KEYS.DRAFT_MESSAGES}:${chatId}`);
      if (saved) {
    ...
    (metadata: Partial<SessionMetadata>) => {
      try {
        ...
        if (current) {
          ...
          localStorage.setItem(metaKey, JSON.stringify({ ...existing, ...metadata }));
        }
      } catch (error) {
...
function updateSessionHistory(entry: SessionHistoryEntry) {
  try {
    ...
    const updated = [
      history.filter((h) => h.chatId !== entry.chatId),
      entry,
    ].flat().slice(-MAX_SESSION_HISTORY);
...
export function importSessions(json: string): boolean {
  try {
    ...
    const merged = [...history, ...data.sessions]
      .reduce((acc, session) => {
        if (!acc.find((s: SessionHistoryEntry) => s.chatId === session.chatId)) {
          acc.push(session);
        }
        return acc;

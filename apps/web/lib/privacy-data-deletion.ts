...
function logDeletionEvent(scope: DeletionScope, count: number): void {
  try {
    const auditLog = JSON.parse(localStorage.getItem("privacy-audit-log") || "[]");
    auditLog.push({
      type: "deletion",
      scope,
      itemCount: count,
 ...
 */
export async function applyRetentionPolicy(retentionDays: number): Promise<DeletionResult> {
...
export async function checkDeletionStatus(): Promise<{
  ...
  try {
    const response = await fetch(`/api/privacy/deletion-status?id=${deletionId}`);

...
- Present balanced conclusions`,
  },
  {
    id: "tutor",
    name: "Learning Tutor",
    description: "Educational explanations that build understanding step by step",
    category: "general",
 ...
 */
export function setChatInstructions(chatId: string, instructions: string): void {
  const current = getCustomInstructions();
...
export function useCustomInstructions() {
  ...
  useEffect(() => {
    ...
    window.addEventListener("custom-instructions-change", handleInstructionsChange);
    window.addEventListener("ai-settings-change", handleAISettingsChange);

    return () => {
      window.removeEventListener("custom-instructions-change", handleInstructionsChange);

...
test.describe("Chat Page - Initial State", () => {
  ...
  test("model selector button is visible", async ({ page }) => {
    await page.goto("/");
    const modelButton = page.locator("button").filter({ hasText: /Gemini|Claude|GPT|Grok/i });
...
test.describe("Chat Message Flow", () => {
  ...
  test("sends message via Enter key", async ({ page }) => {
    ...
    await page.waitForURL(/\/chat\/[\w-]+/, { timeout: 15000 });
  });

  test("sends message via click and Enter produce same result", async ({ page }) => {
    await page.goto("/");
...
test.describe("Model Selector", () => {
  ...
  test("opens model selector popover", async ({ page }) => {
    const modelButton = page.locator("button").filter({ hasText: /Gemini|Claude|GPT|Grok/i }).first();
...
test.describe("Suggested Actions", () => {
  ...
  test("suggested actions are clickable", async ({ page }) => {
    const suggestions = page.locator("[data-testid='suggested-actions'] button");
  ...
  test("suggested actions disappear after sending message", async ({ page }) => {
    const suggestions = page.locator("[data-testid='suggested-actions']");
    ...
    const suggestionsOnChatPage = page.locator("[data-testid='suggested-actions']");
    const isVisible = await suggestionsOnChatPage.isVisible().catch(() => false);
...
test.describe("Stop Generation", () => {
  test.skip(true, "Test skipped - timing-dependent and requires stable API with predictable response times");
  test("stop button appears during message generation", async ({ page }) => {
    await page.goto("/");

    // Send a message that will take time
    await page.getByTestId("multimodal-input").fill("Write a long poem about testing");
    await page.getByTestId("send-button").click();

    // Wait for URL change with timeout
    await page.waitForURL(/\/chat\/[\w-]+/, { timeout: 15000 }).catch(() => {

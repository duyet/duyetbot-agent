...
export class ChatPage {
  ...
  async isThinking() {
    return await this.page.locator("[data-testid='thinking-message']").isVisible().catch(() => false);
  }

  async waitForResponse(timeout = 30000) {
    await this.page.waitForSelector("[data-testid='thinking-message']", { state: "hidden", timeout });
  }

  async waitForMessageCount(count: number, timeout = 30000) {

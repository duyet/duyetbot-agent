  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild>
        <Button variant="ghost" size="icon" title="Search chats">
          <SearchIcon size={18} />
        </Button>
      </DialogTrigger>
...
function SearchResultItem({ result, query, onClick }: SearchResultItemProps) {
  ...
  const getIcon = () => {
    if (result.type === "memory") {
      return <TagIcon size={16} className="text-blue-500" />;
    }
  ...
  return (
    <button
      className={cn(
        "w-full flex items-start gap-3 p-3 hover:bg-accent",
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <p className="font-medium truncate">{highlightText(getTitle(), query)}</p>

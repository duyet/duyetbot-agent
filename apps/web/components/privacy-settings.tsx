...
export function PrivacySettingsDialog() {
	...
	const handleAnonymize = async () => {
		if (
			!window.confirm(
				"Anonymize your data? This will remove personal identifiers but keep your chat history.",
			)
		) {
			return;
		}
	...
	return (
		<Dialog onOpenChange={setIsOpen} open={isOpen}>
			<DialogTrigger asChild>
				<Button size="icon" title="Privacy settings" variant="ghost">
					<ShieldIcon size={18} />
				</Button>
			</DialogTrigger>
		...
		<div className="space-y-6">
			<div className="grid grid-cols-3 gap-4">
				<div className="rounded-lg border p-4">
					<p className="font-bold text-2xl">{dataSummary.sessions}</p>
					<p className="text-muted-foreground text-sm">Chat Sessions</p>
				</div>
				<div className="rounded-lg border p-4">
					<p className="font-bold text-2xl">{dataSummary.memories}</p>
					<p className="text-muted-foreground text-sm">Saved Memories</p>
				</div>
				<div className="rounded-lg border p-4">
					<p className="font-bold text-2xl">
						{(dataSummary.totalSize / 1024).toFixed(1)} KB
					</p>
					<p className="text-muted-foreground text-sm">Local Storage</p>
				</div>
			</div>
			...
			<div className="space-y-4">
				...
				<div className="space-y-2 text-sm">
					<div className="flex items-start gap-2">
						<EyeIcon className="mt-1 text-muted-foreground" size={16} />
						<div>
					...
					<div className="flex items-start gap-2">
						<Trash2Icon className="mt-1 text-muted-foreground" size={16} />
						<div>
					...
					<div className="flex items-start gap-2">
						<CookieIcon className="mt-1 text-muted-foreground" size={16} />
						<div>
	...
	return (
		<div className="space-y-4">
			<p className="text-muted-foreground text-sm">
				Download all your data in a portable format. You can import this data
				later or use it with other services.
			</p>
			...
			<div className="rounded-md bg-muted p-3">
				<p className="text-muted-foreground text-xs">
					<strong>GDPR Right to Data Portability:</strong> You have the right to
		...
		<div className="space-y-4">
			...
			<div className="rounded-md bg-destructive/10 p-3">
				<p className="text-destructive text-xs">
					<strong>Warning:</strong> Deleted data cannot be recovered. Consider
...
function ConsentTab({
	...
	return (
		<div className="space-y-4">
			<p className="text-muted-foreground text-sm">
				Manage your consent for cookies and data processing. Required cookies
				cannot be disabled.
			</p>
			...
			<div className="rounded-md bg-muted p-3">
				<p className="text-muted-foreground text-xs">
					<strong>GDPR Consent:</strong> Consent must be freely given, specific,
					informed, and unambiguous. You can withdraw consent at any time.
				</p>
			</div>
		</div>
	);
...
function RetentionTab({
	...
	return (
		<div className="space-y-4">
			<p className="text-muted-foreground text-sm">
				Configure how long to keep your data. Older data will be automatically
				deleted.
			</p>
			...
			<div className="rounded-md bg-muted p-3">
				<p className="text-muted-foreground text-xs">
					<strong>GDPR Data Minimization:</strong> Personal data should not be
					kept longer than necessary for the purposes for which it is processed.
				</p>
			</div>
		</div>
	);
	...
	return (
		<div className="fixed right-0 bottom-0 left-0 z-50 border-t bg-background p-4 shadow-lg">
			<div className="mx-auto flex max-w-4xl items-center gap-4">
				<CookieIcon className="flex-shrink-0 text-muted-foreground" size={24} />
				<div className="flex-1">
					<p className="font-medium text-sm">Cookie & Privacy Preferences</p>
					<p className="text-muted-foreground text-xs">
				...
				<div className="flex flex-shrink-0 items-center gap-2">
					<Button onClick={handleCustomize} size="sm" variant="ghost">
						Customize
					</Button>
					<Button onClick={handleAcceptNecessary} size="sm" variant="ghost">
						Necessary Only
					</Button>

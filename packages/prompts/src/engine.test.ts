...
import { renderString, renderTemplate } from "./engine.js";
...
describe("Template Engine", () => {
  describe("renderString", () => {
    ...
    it("should handle array length check", () => {
      const template =
        "{% if tools and tools.length > 0 %}Has tools{% else %}No tools{% endif %}";

      expect(renderString(template, { tools: ["bash", "git"] })).toBe(
        "Has tools",
      );

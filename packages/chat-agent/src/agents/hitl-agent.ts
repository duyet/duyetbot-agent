 ...
 */
export type HITLAgentClass<TEnv extends HITLAgentEnv> = typeof Agent<
  TEnv,
  HITLAgentState
> & {
  new (
...
export function createHITLAgent<TEnv extends HITLAgentEnv>(
  ...
  const AgentClass = class HITLAgent extends Agent<TEnv, HITLAgentState> {
      ...
      try {
        ...
        if (toolCalls.length > 0) {
          ...
          if (needsConfirmation.length > 0) {
            ...
            return this.requestConfirmations(
              needsConfirmation,
              traceId,
              updatedMessages,
            );
    ...
    private async executeApproved(): Promise<AgentResult> {
      ...
      // Execute tools
      const result = await executeApprovedTools(
        approved,
        executor,
        {},
        (entry, index, total) => {

 ...
 */
export class StateDO extends Agent<StateDOEnv, StateDOState> implements StateDOMethods {
  override initialState = createInitialStateDOState();
   ...
   */
  async registerBatch(params: RegisterBatchParams): Promise<void> {
   ...
   */
  async markDelegated(params: MarkDelegatedParams): Promise<void> {
    const session = this.state.sessions[params.sessionId];
    if (!session) {
   ...
   */
  async completeBatch(params: CompleteBatchParams): Promise<void> {
    const session = this.state.sessions[params.sessionId];
    if (!session) {
    ...
    const traceParams: LogTraceParams = {
      sessionId: params.sessionId,
      query: '',
   ...
   */
  async logTrace(params: LogTraceParams): Promise<void> {
    const now = Date.now();
   ...
   */
  getStuckSessions(thresholdMs: number = DEFAULT_STUCK_THRESHOLD_MS): SessionState[] {
    const now = Date.now();
    const stuckSessions: SessionState[] = [];
   ...
   */
  getTraces(sessionId: string, limit = 10): ExecutionTrace[] {
   ...
   */
  async forceRecover(sessionId: string): Promise<RecoveryResult> {
   ...
   */
  private async clearSession(sessionId: string, reason: string): Promise<void> {
    const session = this.state.sessions[sessionId];
    if (!session) {
      return;
    }
   ...
   */
  private async notifyUser(session: SessionState, message: string): Promise<void> {
    if (!session.responseTarget) {
      return;
    }
   ...
   */
  private async sendTelegramNotification(session: SessionState, message: string): Promise<void> {
    // Access env via type assertion (required by Agent base class pattern)
    const env = (this as unknown as { env: StateDOEnv }).env;

...
export class SchedulerDO {
  ...
  private taskExecutorUrl: string | undefined;

  constructor(
    private ctx: SchedulerDOContext,
    private env: SchedulerDOEnv
  ) {
   ...
   */
  private shouldWakeUp(now: number, readyTasks: ScheduledTask[]): WakeUpDecision {
   ...
   * @param now - Current timestamp
   */
  private async processReadyTasks(readyTasks: ScheduledTask[], now: number): Promise<void> {
    let executedCount = 0;

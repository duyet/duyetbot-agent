...
import { createThinkingRotator, getDefaultThinkingMessages } from '../format.js';
/**
...
export class BatchProcessor {
   ...
   */
  async process(batch, deps, env, _platform) {
    ...
    try {
      ...
      const ctx = this.buildTransportContext(firstMessage, combinedText, env, deps.transport);
   ...
   */
  async executeProcessing(combinedText, batch, firstMessage, _ctx, deps, rotator) {
    try {
   ...
   */
  buildTransportContext(message, text, env, _transport) {
    const envRecord = env;
   ...
   */
  buildAgentContext(message, query, env) {
    const envRecord = env;
    const platform = envRecord.PLATFORM || 'api';
    return {
      query,
      platform: platform,

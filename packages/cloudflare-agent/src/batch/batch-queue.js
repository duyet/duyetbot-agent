...
import { createInitialBatchState, isDuplicateInBothBatches } from './types.js';
/**
...
export class BatchQueue {
   ...
   */
  constructor(_getState, _setState, stuckDetector, _config) {
  ...
  promoteToActive() {
    ...
    const { pendingBatch: _removed, ...restOfState } = queueState;
    this._setState({
      ...restOfState,
   ...
   * @internal
   */
  _clearActiveBatch(now) {
    const queueState = this._getState();
    const { activeBatch: _removed, ...restOfState } = queueState;
    this._setState({
      ...restOfState,
      updatedAt: now,
    });
   ...
   * @internal
   */
  _shouldScheduleAlarm(pendingBatch, activeBatch, recoveredFromStuck) {

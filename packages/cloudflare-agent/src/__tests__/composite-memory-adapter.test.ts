...
describe('CompositeMemoryAdapter', () => {
  ...
  describe('getMemory - parallel strategy', () => {
    it('should merge results from both adapters', async () => {
      ...
      (primary.getMemory as any).mockResolvedValue({
        sessionId: 'session-1',
        messages: [
          { role: 'user', content: 'from primary', timestamp: 1000 },
        ],
      });
      (secondary.getMemory as any).mockResolvedValue({
        sessionId: 'session-1',
        messages: [
          { role: 'assistant', content: 'from secondary', timestamp: 2000 },
        ],
      });
    ...
    it('should deduplicate messages by content', async () => {
      ...
      (primary.getMemory as any).mockResolvedValue({
        sessionId: 'session-1',
        messages: [
          { role: 'user', content: 'duplicate message', timestamp: 1000 },
        ],
      });
      (secondary.getMemory as any).mockResolvedValue({
        sessionId: 'session-1',
        messages: [
          { role: 'user', content: 'duplicate message', timestamp: 2000 },
        ],
      });
  ...
  describe('saveMemory - all strategies', () => {
    it('should write to both adapters (primary sync, secondary async)', async () => {
      ...
      (primary.saveMemory as any).mockResolvedValue({ savedCount: 1, sessionId: 'session-1', updatedAt: Date.now() });
      (secondary.saveMemory as any).mockResolvedValue({ savedCount: 1, sessionId: 'session-1', updatedAt: Date.now() });
  ...
  describe('searchMemoryByQuery', () => {
    it('should use secondary for mem0-for-search strategy', async () => {
      ...
      const mockResults = [
        { id: 'mem-1', content: 'test', category: 'fact', score: 0.9 },
      ];

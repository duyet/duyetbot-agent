 ...
 * @see https://core.telegram.org/bots/api#markdownv2-style
 */
export function smartEscapeMarkdownV2(text) {
  const result = [];
  let i = 0;
  while (i < text.length) {
 ...
 */
function matchFormattedLink(text, start, marker) {
 ...
 */
function escapeLinkTextForFormattedLink(text) {
 ...
 */
function isMarkdownV2Special(char) {
  return '_*[]()~`>#+-=|{}.!\\'.includes(char);
}
/**
 * Check if character at position is a formatting marker to preserve
 */
function isFormattingMarker(text, pos) {
  const char = text[pos];
  // Bold: *
  if (char === '*') {
 ...
 */
function isBalancedMarker(text, pos, marker) {
 ...
 * - URL: only escape ) and \
 */
function formatLink(linkText, url) {
 ...
 */
function escapeInsideCodeBlock(codeBlock) {
 ...
 */
function escapeInsideInlineCode(inlineCode) {
  // Keep the ` markers, escape ` and \ inside
 ...
 */
function formatAgentTiming(name, durationMs, status, tokenUsage) {
  if (status === 'running') {
    return `${name} (running)`;
  }
  const parts = [];
 ...
 */
function formatRoutingFlow(debugContext) {
 ...
 */
function formatMetadata(metadata, escapeFn = escapeHtml) {
  if (!metadata) {
    return '';
  }
 ...
 * ```
 */
export function formatDebugFooter(debugContext) {
 ...
 */
function formatWorkersMarkdownV2(workers) {
  if (!workers?.length) {
    return '';
  }
  return workers
 ...
 * ```
 */
export function formatDebugFooterMarkdownV2(debugContext) {
  if (!debugContext?.routingFlow?.length) {
    return null;
  }
  const flow = formatRoutingFlow(debugContext);
 ...
 * ```
 */
export function formatProgressiveDebugFooter(debugContext) {
  if (!debugContext?.routingFlow?.length) {
    return null;
  }
  const flow = formatRoutingFlow(debugContext);
 ...
 * ```
 */
export function formatDebugFooterMarkdown(debugContext) {
  if (!debugContext?.routingFlow?.length) {
    return null;
  }
  const flow = formatRoutingFlow(debugContext);

 ...
 */
export function detectGitHubTaskType(task) {
 ...
 */
function getGitHubInstructions(taskType) {
  const instructions = {
    pr_review: `
## PR Review Checklist
1. **Code Quality**
 ...
 */
function buildGitHubPrompt(step, dependencyContext, defaultOwner, defaultRepo) {
 ...
 */
function parseGitHubResponse(content, expectedOutput) {
 ...
 * ```
 */
export function createGitHubWorker(config) {
  const baseConfig = {

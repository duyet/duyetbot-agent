 ...
 */
export async function aggregateResults(plan, executionResult, config) {
 ...
 */
async function synthesizeWithLLM(plan, executionResult, stepOutputs, config) {
 ...
 */
function formatFailureResponse(plan, errors) {
 ...
 */
function formatSimpleAggregation(plan, executionResult, stepOutputs) {
  const sections = [];
 ...
 */
export function quickAggregate(plan, executionResult) {
  const stepOutputs = [];
  const errors = [];
  for (const step of plan.steps) {
    const result = executionResult.results.get(step.id);
 ...
 */
export function extractKeyFindings(result) {
  const findings = [];
  ...
  for (const output of result.stepOutputs) {
    ...
    if (typeof output.output === 'string') {
      ...
      if (obj.findings && Array.isArray(obj.findings)) {
        findings.push(...obj.findings.slice(0, 3));
      }

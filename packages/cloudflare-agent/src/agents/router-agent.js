...
import { sendPlatformResponse } from '../platform-response.js';
import { determineRouteTarget, hybridClassify } from '../routing/index.js';
/**
...
export function createRouterAgent(config) {
  ...
  const AgentClass = class RouterAgent extends BaseAgent {
    initialState = {
      ...createBaseState(),
      sessionId: '',
     ...
     */
    async scheduleExecution(ctx, responseTarget) {
      ...
      try {
        ...
        if (result.debug?.workers && result.debug.workers.length > 0) {
          debugContext.workers = result.debug.workers.map((w) => ({
            ...
            status:
              w.status === 'success' ? 'completed' : w.status === 'failed' ? 'error' : 'error',
            error: w.error,
          }));
        }

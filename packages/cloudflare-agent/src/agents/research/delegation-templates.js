 ...
 */
export function buildDelegationPrompt(context) {
  const sections = [];
  // Objective section
 ...
 */
function getOutputFormatInstructions(format) {
  switch (format) {
    case 'text':
 ...
 */
export function getSubagentSystemPrompt(type) {
  switch (type) {
    case 'research':
 ...
 */
export function buildSubagentPrompt(type, delegationContext) {
 ...
 */
export function getDefaultToolGuidance(type) {
  switch (type) {
    case 'research':
      return [
 ...
 */
export function getDefaultBoundaries(type) {
  const common = [
...
export function formatDependencyContext(results) {
  ...
  const parts = ['The following tasks have been completed:'];

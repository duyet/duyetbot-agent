...
describe('ObservabilityStorage', () => {
  ...
  describe('writeEvent', () => {
    ...
    it('should serialize agents array to JSON', async () => {
      ...
      const agentsJson = bindCall.find(
        (arg: unknown) => typeof arg === 'string' && arg.includes('"name":"router"')
      );
  ...
  describe('getEventsBySource', () => {
    it('should filter by app source', async () => {
      ...
      expect(mockDb.prepare).toHaveBeenCalledWith(expect.stringContaining('WHERE app_source = ?'));
      expect(mockDb._statement.bind).toHaveBeenCalledWith('telegram-webhook', 25);
    });
  });
  ...
  describe('getRecentErrors', () => {
    it('should filter by error status and time', async () => {
      ...
      expect(mockDb.prepare).toHaveBeenCalledWith(expect.stringContaining('triggered_at > ?'));
    });
  });

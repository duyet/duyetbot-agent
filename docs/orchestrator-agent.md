...
```mermaid
graph TD
  A[Router Routes<br/>high complexity] --> B[OrchestratorAgent]
  B --> C[Planner: LLM Plan]
  C --> D[Executor: Group Levels]
  D --> E[Parallel Workers<br/>Level 1]
  E --> F[Parallel Workers<br/>Level 2]
  F --> G[Aggregator: Synthesize]
  G --> H[Final Response]
```
